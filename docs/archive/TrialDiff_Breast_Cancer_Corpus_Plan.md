...
> **Archived working document (2026-05-20).** The corpus selector defined here remains authoritative as implemented in `trialdiff/corpus.py` (CLI `select-corpus`); the selected NCT lists are committed under `corpora/`. See `../../CORPUS.md` for the snapshots derived from it.
