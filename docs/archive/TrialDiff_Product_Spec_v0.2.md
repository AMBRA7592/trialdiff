# TrialDiff Product Spec v0.2

> **Archived historical design document (2026-05-20).** The severity-as-review-priority framing in this spec was retracted after the v0.2/v0.2.1 calibration failed its pre-registered gate — see `../../SEVERITY_DECOUPLING_v0.2.1.md`. Severity is deterministic, uncalibrated triage metadata. Spec numbering does not track releases; see `../../VERSIONS.md`.

**Date:** 2026-05-20
**Status:** Build-facing draft
**Supersedes:** v0.1 concept/spec from the original TrialDiff conversation
**Core revision:** TrialDiff no longer needs to build the primary diff engine. It should consume structured ClinicalTrials.gov history patches where available, classify materiality deterministically, and run its own snapshot pipeline from day one for resilience.
...
- enrollment count changes by more than 20 percent
- enrollment type changes from anticipated to actual
- eligibility criteria change after recruitment has started
- `whyStopped` is empty, vague, or uses low-information phrases
